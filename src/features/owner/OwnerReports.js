...
const loadJsPDF = () => import("https://cdn.jsdelivr.net/npm/jspdf@2.5.1/dist/jspdf.umd.min.js");
// ─── Constants ────────────────────────────────────────────────────────────────
...
const handleDownloadExcel = async () => {
  ...
  const handleDownloadReportPdf = async () => {
    ...
    try {
      ...
      const { jsPDF } = (await loadJsPDF()).jspdf;
      ...
      doc.text("SPARX EV — MONTHLY REPORT", 14, 8);
      ...
      doc.setFontSize(16);
      doc.setFont("helvetica", "bold");
      doc.text("Owner Revenue Statement", 14, y);
      ...
      doc.setFont("helvetica", "normal");
      doc.setTextColor(...gray);
      doc.text("Generated by Sparx EV — VIZ-Smart Charging, a brand of Vjra Technologies LLP", 14, y);
      doc.text(`Generated on: ${new Date().toLocaleDateString("en-IN")}`, 196, y, { align: "right" });

      doc.save(`Sparx_Report_${period.replace(" ", "_")}.pdf`);
